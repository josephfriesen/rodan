...
class Day7Solution extends SolutionBuilder {
    ...
    binaryStrings(n: number): Array<string> {
        if (n < 0) {
            throw new TypeError("Expected non-negative integer n, how can you have a negative-length string come on bozo.");
        }
    ...
    checkCalibrationValidity(c: Calibration, stringBuilder: (n: number) => Array<string> = this.binaryStrings): boolean {
        ...
        for (const string of strings) {
            ...
            const operationQueue: Queue = this.generateQueue(this.binaryOperations(string));
