...
292: 11 6 16 20`;
  ...
  it("should have a list of calibrations based on the lines of the input", () => {
    expect(solution.calibrations[0]).toEqual({ goal: 190, inputs: [10, 19], operations: 1 });
    expect(solution.calibrations[7]).toEqual({ goal: 21037, inputs: [9, 7, 18, 13], operations: 3 });
  });
    ...
    it("should have a method binaryStrings that gives all the binary strings of length n", () => {
        ...
        expect(solution.binaryStrings(1)).toEqual(expect.arrayContaining(["0", "1"]));
        expect(solution.binaryStrings(2).length).toBe(2**2)
        expect(solution.binaryStrings(2)).toEqual(expect.arrayContaining(["00", "01", "10", "11"]));
        expect(solution.binaryStrings(3).length).toBe(2**3);
        expect(solution.binaryStrings(3)).toEqual(expect.arrayContaining(["000", "001", "010", "100", "101", "110", "111"]));
    });
    ...
    it("should have a method checkCalibrationValidity to determine if there is a possible series of consecutive binary operations such that after applying operations to inputs we obtain goal", () => {
        expect(solution.checkCalibrationValidity(solution.calibrations[0])).toBe(true);

        expect(solution.checkCalibrationValidity(solution.calibrations[1])).toBe(true);
        expect(solution.checkCalibrationValidity(solution.calibrations[2])).toBe(false);
        expect(solution.checkCalibrationValidity(solution.calibrations[3])).toBe(false);
        expect(solution.checkCalibrationValidity(solution.calibrations[4])).toBe(false);
        expect(solution.checkCalibrationValidity(solution.calibrations[5])).toBe(false);
        expect(solution.checkCalibrationValidity(solution.calibrations[6])).toBe(false);
        expect(solution.checkCalibrationValidity(solution.calibrations[7])).toBe(false);
        expect(solution.checkCalibrationValidity(solution.calibrations[8])).toBe(true);
    });
