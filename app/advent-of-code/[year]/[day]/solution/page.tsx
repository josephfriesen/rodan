...
export default async function AOCSolutionsListPage({ params }: { children: React.ReactNode, params: { year: string, day: string } }) {
      ...
      {puzzle.solutions.length > 0 && (
        <ul className={clsx(utilStyles.list)}>
          {puzzle.solutions.map((solution) => {
            return (
              <li key={solution.id}>
                ID: <Link href={solution.url}>{solution.id}</Link>{" "}
                Created {solution.createdAt.toLocaleString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </li>
            )
          })}
        </ul>
      )}
      <LinkButton clickCallback={handleNewSolutionClick}>Create new solution for day {day}</LinkButton>
    </section>
  )
}