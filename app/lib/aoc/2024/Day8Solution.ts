...
export default class Day8Solution extends SolutionBuilder {
    ...
    constructor(input) {
        ...
        this.setUniqueAntinodes(this.extendedUniqueAntinodes, this.getExtendedAntinodes);
    }

    private initializeMatrix(): void {
        this.matrix = new Matrix(this.input.split("\n").filter(Boolean).map((line) => line.split("")));
    }
    ...
    static entryDistance(e: [number, number], f: [number, number]): [number, number] {
        return [f[0] - e[0], f[1] - e[1]];
    }
    getAntinodes(e: [number, number], f: [number, number]): Array<[number, number]> {
        ...
        return [this.matrix.inbounds(fnode) && fnode, this.matrix.inbounds(enode) && enode].filter(Boolean);
    } 

    getExtendedAntinodes(e: [number, number], f: [number, number]): Array<[number]> {
    ...
    private setUniqueAntinodes(uniqueAntinodes: Set<[number, number]> = this.uniqueAntinodes, nodeCalc: (e: [number, number], f: [number, number]) => Array<[number, number]> = this.getAntinodes): void {
        ...
        for (const coordinates of this.nodeMap.values()) {
            ...
            while (!entryQueue.isEmpty()) {
                ...
                for (const comparison of entryQueue.list) {
                    const antinodes: Array<[number, number]> = getAntinodes(current, comparison);
